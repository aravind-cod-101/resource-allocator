...
def upgrade():
    ...
    op.create_check_constraint("ck_tasks_valid_date_range","tasks","start_date <= end_date")
    op.create_check_constraint("ck_resource_availability_valid_range","resource_availability","available_from <= available_to")

def downgrade():
    op.drop_constraint("ck_resource_availability_valid_range","resource_availability",type_="check")
